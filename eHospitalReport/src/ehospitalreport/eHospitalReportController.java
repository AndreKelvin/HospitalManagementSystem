 ...
 * @author Andre Kelvin
 */
public class eHospitalReportController implements Initializable {
    ...
    private class GenerateTotalTask extends Task<Object> {
        ...
        protected Object call() throws Exception {
            ...
            try {
                ...
                ps = EHospitalDB.getCon().prepareStatement("SELECT Count(PATIENT_NAME) FROM PATIENT WHERE ADMITED_DATE Between ? and ?");
                ...
                ps = EHospitalDB.getCon().prepareStatement("SELECT Count(NAME) FROM DEATH Where DATE Between ? and ?"
                        + "Union All "
                ...
                ps = EHospitalDB.getCon().prepareStatement("SELECT DATE_OF_BIRTH from PATIENT Where ADMITED_DATE Between ? and ?");
                ps.setString(1, dateFrom.getValue().toString());
                ps.setString(2, dateTo.getValue().toString());
                rs = ps.executeQuery();
                ...
                Platform.runLater(() -> {
                    EHospitalDialog.dialogAlert(Alert.AlertType.ERROR, "Database connection failure", tabPane.getScene().getWindow());
                });
            }
    ...
    private void generateTotal() {
        if (dateFrom.getValue() == null || dateTo.getValue() == null) {
            EHospitalDialog.dialogAlert(Alert.AlertType.ERROR, "Invalid! Select Dates", dateFrom.getScene().getWindow());
        } else {
            tabPane.setDisable(true);
            spinner.setVisible(true);

            GenerateTotalTask task = new GenerateTotalTask();
