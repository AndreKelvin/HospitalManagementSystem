...
public class EditPatientController implements Initializable {
        ...
        try {
            ...
            comboWardChange = (ChangeListener) (ObservableValue observable, Object oldValue, Object newValue) -> {
                try {
                    ...
                    ps = EHospitalDB.getCon().prepareStatement("Select Beds From WARD Where Ward=?");
            ...
            comboFloorChange = (ChangeListener) (ObservableValue observable, Object oldValue, Object newValue) -> {
                try {
                    ...
                    ps = EHospitalDB.getCon().prepareStatement("Select Room From ROOM Where Floor=?");
                    ps.setString(1, newValue.toString());
                    rs = ps.executeQuery();
    ...
    public void populateComboBoxs() throws SQLException {
        ...
        ps = EHospitalDB.getCon().prepareStatement("Select Ward From WARD");
        rs = ps.executeQuery();
        ...
        ps = EHospitalDB.getCon().prepareStatement("Select Floor From ROOM");
        rs = ps.executeQuery();
    ...
    public void setSelectedPatientNameAndID(String selectedPatientName, int selectedPatientID) throws SQLException {
        ...
        ps = EHospitalDB.getCon().prepareStatement("Select * From PATIENT Where Patient_Name=? and ID=?");
    ...
    private class EditTask extends Task<Object> {
        ...
        protected Object call() throws Exception {
            ...
            try {
                ps = EHospitalDB.getCon().prepareStatement("Update PATIENT Set Patient_Name=?,Admited_Date=?,Date_Of_Birth=?,Gender=?,Marital_Status=?,"
                ...
                if (comboPatientCat.getValue().toString().contentEquals("Discharge Patient")) {
                    ps = EHospitalDB.getCon().prepareStatement("Insert Into DISCHARGED_DATE Values(?,?)");
                    ps.setString(1, selectedPatientName);
                    ...
                    EHospitalBridge.comboBoxBillPatientName.getItems().add(selectedPatientName);
                } else {
    ...
    private void saveAction() {
        ...
        if (patientName.isEmpty() || phone.isEmpty() || address.isEmpty() || city.isEmpty() || diagnosis.isEmpty() || date.getValue() == null
            ...
            task.setOnSucceeded((event) -> {
                ...
                EHospitalDialog.dialogAlert(Alert.AlertType.INFORMATION, "Save Successfull", textAddress.getScene().getWindow());
